...
// Slash Commands
const commands = [
  new SlashCommandBuilder().setName("setup").setDescription("Pasang panel tiket di channel ini.").setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
  new SlashCommandBuilder().setName("close").setDescription("Tutup tiket (staff only)").setDefaultMemberPermissions(PermissionFlagsBits.ManageChannels),
  new SlashCommandBuilder().setName("status").setDescription("Ubah status tiket (staff only)").addStringOption(o=>o.setName("to").setDescription("paid/processing").setRequired(true).addChoices({name:"paid",value:"paid"},{name:"processing",value:"processing"})).setDefaultMemberPermissions(PermissionFlagsBits.ManageChannels),
...
function ticketIntroEmbed() {
  return new EmbedBuilder()
    ...
    .setDescription(`Silakan lakukan pembayaran:\n• QRIS: ${QRIS_URL}\n• PayPal: ${PAYPAL_URL}\n\nSetelah bayar WAJIB kirim Transaction ID pakai tombol di bawah.`)
...
async function composeStatusEmbed(guildId,ticket){
  ...
  if(ticket.queue_number){
    const { data: posData } = await supabase.from("tickets").select("id,queue_number").eq("guild_id",guildId).in("status",["paid","processing"]).lte("queue_number",ticket.queue_number);
...
// Ready
client.once("ready", async()=>{ console.log(client.user.tag,"online"); await registerCommands(); });
  ...
  try {
    if(interaction.isChatInputCommand()){
      if(interaction.commandName==="setup"){
        ...
        const embed=new EmbedBuilder().setTitle("LimeHub — Ticket Panel").setDescription("Klik tombol di bawah untuk membuat tiket.\nBot akan kasih instruksi bayar + antrian realtime.").setFooter({ text:"made by @unstoppable_neid"});
      ...
      if(interaction.commandName==="on"){
        if(!interaction.member.roles.cache.has(STAFF_ROLE_ID)) return interaction.reply({content:"Buat staff aja",ephemeral:true});
      ...
      if(interaction.commandName==="close"){
        ...
        await interaction.reply({content:"Tiket ditutup ✅",ephemeral:true});
      }
    }
    if(interaction.isButton()){
      if(interaction.customId==="create_ticket"){
        const name=`ticket-${interaction.user.username}`.toLowerCase().replace(/[^a-z0-9-]/g,"");
        const chan=await interaction.guild.channels.create({name,type:ChannelType.GuildText,permissionOverwrites:[
    ...
    if(interaction.isModalSubmit() && interaction.customId==="txid_modal"){
      // TODO: isi logic TXID sama kayak versi Mongo → cek duplikat, simpan, update status
    }
